...
- (id)init {
	...
	if(self != nil) {
        ...
        /*
         Refrence:
         Create The Remote Notification Playload.
         ...
         @{@"aps":
                ...
                @"sound":@"default"}}
         */
        NSDictionary *playload = @{@"aps":
                                       @{@"alert":@"Message received from Bob.",
                                         @"sound":@"default"}};
        NSError *error = nil;
        NSData *data = [NSJSONSerialization dataWithJSONObject:playload options:NSJSONWritingPrettyPrinted error:&error];
        if (error) {
            NSLog(@"%@", error);
            return self;
        }
